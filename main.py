...
class TNT(nn.Module):
  ...
  def forward(self, inputs, lens):
    ...
    net = self.dropout1(self.act1(self.fc1(sent)))
    out = self.fc2(net)
    return out
...
args.add_argument('--convchannel', type=int, default=16)
args.add_argument('--tsize', type=int, default=16)
args.add_argument('--lrstep', type=int, default=300)
args.add_argument('--level', type=int, default=3)
args.add_argument('--attention', type=bool, default=False)
config = args.parse_args()

DATASET_PATH = './data/'
dataset = AgDataset(DATASET_PATH, config.strmaxlen)
...
if config.mode == 'train':
  ...
  scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=config.lrstep, factor=0.95, verbose=True, mode='min')
